(function() {
    ...
    function loadOutline() {
        ...
        if (!loading) {
            ...
            var u = new Ajax.Updater(document.getElementById("side-panel"),
                rootURL+"/descriptor/com.agfa.jenkins.ataf.AtafTestNote/outline",
                {insertion: Insertion.Bottom, onComplete: function() {
                    ...
                    outline = document.getElementById("ataf-tests-body");
                    loading = false;
                    queue.each(handle);
                }});
        }
    ...
    function handle(e) {
        if (loadOutline()) {
            ...
            var id = "ataf-test-"+(iota++);
            outline.appendChild(parseHtml("<li><a href='#"+id+"' class=\""+e.getAttribute("class")+"\">"+e.getAttribute("test") +"</a></li>"))
